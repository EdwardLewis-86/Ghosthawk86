...
# Define output path
output_path = "C:/Users/edwardl/OneDrive - Motus Corporation/Documents/3. RStudio_Python Outputs/Existing Claim Data Analyses 5 year non importer/"
QQ_output_path = "C:/Users/edwardl/OneDrive - Motus Corporation/Documents/3. RStudio_Python Outputs/Existing Claim Data Analyses 5 year non importer/QQ Plots/"

# Define input path
input_path = "C:/Users/edwardl/OneDrive - Motus Corporation/Documents/2. RStudio_Python Inputs/5 year non importer/"
...
data_path = "C:/Users/edwardl/OneDrive - Motus Corporation/Documents/2. RStudio_Python Inputs/5 year non importer/claims_policy_data.xlsx"
...
simulated_df = pd.DataFrame(simulated_claims,
                            columns=["Vehicle Make", "Claim Reason", "Claim Period", "Claim Cost", "Benefit Limit"])
...
simulated_claims_df = pd.DataFrame(simulated_claims,
                                   columns=["Vehicle Make", "Claim Reason", "Claim Period", "Claim Cost",
                                            "Benefit Limit"])
...
simulated_claims_df["Frequency Source"] = np.where(simulated_claims_df["Claim Reason"].isin(weights_df["Claim Reason"]),
                                                   "Actual", "Simulated")

#################################################### Summary##################################################################
summary_stats = simulated_claims_df.groupby("Claim Reason")["Claim Cost"].describe(
    percentiles=[.25, .5, .75]).reset_index()
summary_stats.rename(
    columns={"mean": "Mean", "std": "Std Dev", "min": "Min", "25%": "P25", "50%": "Median", "75%": "P75", "max": "Max"},
    inplace=True)

claim_counts = simulated_claims_df["Claim Reason"].value_counts().reset_index()
claim_counts.columns = ["Claim Reason", "Total Claims"]

summary_stats = pd.merge(pd.DataFrame({"Claim Reason": list(all_reasons)}), summary_stats, on="Claim Reason",
                         how="left")
summary_stats = pd.merge(summary_stats, claim_counts, on="Claim Reason", how="left")
summary_stats[["Mean", "Std Dev", "Min", "P25", "Median", "P75", "Max", "Total Claims"]] = summary_stats[
    ["Mean", "Std Dev", "Min", "P25", "Median", "P75", "Max", "Total Claims"]].fillna(0)

# Merge actual frequency and exposure
actual_inputs = actual_freq_df[["Claim Reason", "Exposure_Months", "Actual Frequency"]].rename(
    columns={"Exposure_Months": "Actual Exposure"})
...
summary_stats["Data Source"] = summary_stats["Claim Reason"].apply(
    lambda x: "Actual" if x in weights_df["Claim Reason"].values else "Simulated")
...
summary_stats = summary_stats[[col for col in ordered_cols if col in summary_stats.columns]].sort_values(
    by="Claim Reason")
