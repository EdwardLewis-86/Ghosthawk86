...
set @ValuationMonth = '01-Sep-2025';
set @Insurer = 'Hollard';
...
Drop table if exists		#Pol_final;
...
Into						#pol --11 027
From						[UPP].[dbo].[SAW_UPP_202509] u -->Update
...
CROSS APPLY ( --Only for new business
    SELECT 
        FilterDate = 
            CASE 
                WHEN DATEADD(MONTH, DATEDIFF(MONTH, 0, ISNULL(c.pol_crt_date, u.POL_CreateDate)), 0) 
                     < DATEADD(MONTH, DATEDIFF(MONTH, 0, p.POL_OriginalStartDate), 0)
                THEN DATEADD(MONTH, DATEDIFF(MONTH, 0, ISNULL(c.pol_crt_date, u.POL_CreateDate)), 0)
                ELSE DATEADD(MONTH, DATEDIFF(MONTH, 0, p.POL_OriginalStartDate), 0)
            END
) AS derived
...
Where						1 = 1
						...
						and derived.FilterDate BETWEEN '2024-07-01' AND '2025-09-01'; 
							-- and u.POL_PolicyNumber = 'HWTY177193POL' ;
...
Select						#pol2.POL_PolicyNumber,
							...
							#pol2.ValuationMonth,
							#pol2.PhasingCurve,
							#pol2.PremiumFrequency,
							#pol2.CellCaptive,
							#pol2.Underwriter,
							#pol2.CSMPhasingKey,
							#pol2.ProductCategory,
							#pol2.GroupingYear,
							#pol2.IRMonth,
...
-- Add forward rates
Select						#pol6.*,
							Dateadd(month, -1, #pol6.IRMonth) LockedInYieldCurveMonth,
							case when Dateadd(month, -1, #pol6.IRMonth) < @MinYieldCurve then @DefaultYield
...
From						#pol6
							...
							on r.valuationdate = Dateadd(month, -1, #pol6.IRMonth)
...
Select						p.POL_PolicyNumber, p.ValuationMonth,
p.ExitRate, p.ExitRateFutureActive, p.ExposureMonth, p.RowN,p.CellCaptive,p.Underwriter,p.CSMPhasingKey,p.PhasingCurve ,p.ProductCategory, p.IRMonth, p.LockedInYieldCurveMonth,p.GroupingYear,p.PremiumFrequency, p.ContractBoundary,p.SurvStart, 
p.WithdrawalBenefit,p.ForwardRateLockedIn,p.DF_Arrears ,p.DF_Advance ,p.SurvEnd, p.EPremium,p.EWithdrawal, p.EDisbursements, p.EClaims, p.EPVClaims, p.EPVDisbursements, p.EPVWithdrawal,p.EPVFCI, p.EPVFCO,
Case
								When p.CellCaptive = 'Auto Pedigree' Then '01-HPS-IW-WW-AP1-81-WAP'
								When p.CellCaptive = 'CMH' Then '01-HPS-IW-WW-CMH-81-WCM'
								When p.CellCaptive = 'Motus OEM' Then '01-HPS-IW-WW-IMP-81-WIM'
								When p.CellCaptive = 'IEMAS' Then '01-HPS-IW-WW-IET-81-WIE'
								When p.CellCaptive = 'M-Sure Mobility' Then '01-HPS-IW-WW-ELS-81-WEL'
								When p.CellCaptive = 'Master Cell' Then '01-HPS-IW-WW-000-81-W00'
								When p.CellCaptive = 'Kempston Group' Then '01-HPS-IW-WW-KMG-81-WKM'
								When p.CellCaptive = 'Wesbank (WESB)' Then '01-HPS-IW-WW-WPI-81-WW3'
								When p.CellCaptive = 'Maritime Motors' Then '01-HPS-IW-WW-MTM-81-WMT'
								When p.CellCaptive = 'Meyers Group' Then '01-HPS-IW-WW-MEY-81-WME'
								When p.CellCaptive = 'Motus Imports' Then '01-HPS-IW-WW-AMH-81-WAM'
							Else Null
						End IFRS_Group,
				--		ROW_NUMBER() OVER (PARTITION BY POL_PolicyNumber ORDER BY (SELECT NULL)) AS RowAsc,
         --  COUNT(*) OVER (PARTITION BY POL_PolicyNumber) AS TotalRows,
...
Into						#IR0
From						#pol11 p--where pol_policynumber = 'HWTYM000704POL'
left join (Select distinct POL_PolicyNumber, CellCaptive, CreateMonth, IRMonth, GroupingYear from #pol2) as t
							on p.POL_PolicyNumber = t.POL_PolicyNumber

						


Group by							p.POL_PolicyNumber, p.ValuationMonth,
p.ExitRate, p.ExitRateFutureActive, p.ExposureMonth, p.RowN,p.CellCaptive,p.Underwriter,p.CSMPhasingKey,p.PhasingCurve ,p.ProductCategory, p.IRMonth, p.LockedInYieldCurveMonth,p.GroupingYear,p.PremiumFrequency, p.ContractBoundary,p.SurvStart, 
p.WithdrawalBenefit,p.ForwardRateLockedIn,p.DF_Arrears ,p.DF_Advance ,p.SurvEnd, p.EPremium,p.EWithdrawal, p.EDisbursements, p.EClaims, p.EPVClaims, p.EPVDisbursements, p.EPVWithdrawal,p.EPVFCI,p.EPVFCO,
Case
								When p.CellCaptive = 'Auto Pedigree' Then '01-HPS-IW-WW-AP1-81-WAP'
...
--TEst with amanda
--SELECT *, 
--       DATEADD(MONTH, i.RowAsc - i.TotalRows, i.ContractBoundary) AS CalendarMonth ,
--	    ISNULL(c.LevelOfCover, 0) LevelOfCover,
--                            ISNULL(c.LevelOfCover, 0) * i.SurvStart CoverageUnits
--	   FROM #IR0 i
--	    left join CoverageUnits c
--                            on i.CSMPhasingKey = c.CSMPhasingKey
--                            and i.ExposureMonth = c.ExposureMonth 
--	   ORDER BY i.POL_PolicyNumber, i.RowAsc
--Extract here-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Select						i.*,--c.valuationMonth, 
t.CreateMonth,-- t.IRMonth,
c.LevelOfCover, c.LevelOfCover * i.SurvStart CoverageUnits,
                            CONCAT (
							Case
								...
								Else Null
							End, '-',Profitability,'-',CAST(i.GroupingYear as varchar(4)))  IFRSGroup,
							ROW_NUMBER() over ( partition by i.POL_PolicyNumber , i.ValuationMonth ,i.RowN  order by i.RowN asc) as rn
...
From						#pol11 i
							...
							on i.Pol_PolicyNumber = c.POL_PolicyNumber
							and i.mth = c.ValuationMonth
							and i.mth = c.CalendarMonth
						
							Left join (Select distinct POL_PolicyNumber, case when AtIR_LossComponent < 0 then 'ONRUS' else 'NSPBO' end Profitability from #IR1) p
					        on p.POL_PolicyNumber = i.POL_PolicyNumber 
							where 1 = 1 ;
		SELECT * FROM #Pol_final
		WHERE 1 =1
	AND rn = '1' ;
