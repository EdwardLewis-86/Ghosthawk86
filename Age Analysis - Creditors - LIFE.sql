	...
	INTO #tmp_SourceTable
		...
		WHERE ATN_AccountTransactionSet_ID = AccountTransactionSet_ID
			...
			AND ATS_Insurer_Id NOT IN (
				...
				'0F2B8071-42D3-4150-A25E-F58576321AF3',
				'28BEBA82-5AD3-49A7-A9F0-714542B6B2A8',
				'2C004120-3D4E-4B89-BA61-FFBE7E033780',
				'4E6D2570-B096-4B7A-9FE5-D12A77B47670'
			)
...
BEGIN /* Result */
	SELECT
		...
		[SALES_BRANCH],
		AGENT_TYPE,
		--#tmp_Summation.TOTALDEBITS AS [Total Debits],
		--#tmp_Summation.TOTALCREDITS AS [Total Credits],
		--SUM(#tmp_Summation.Future) AS [Future Dated],
		SUM(#tmp_Summation.[Current]) AS [Current],
		SUM(#tmp_Summation.[30 Days]) AS [30 Days],
		SUM(#tmp_Summation.[60 Days]) AS [60 Days],
		SUM(#tmp_Summation.[90 Days]) AS [90 Days],
		SUM(#tmp_Summation.[120 Days]) AS [120 Days],
		SUM(#tmp_Summation.[150 Days]) AS [150 Days],
		SUM(#tmp_Summation.[180 Days]) AS [180 Days],
		SUM(#tmp_Summation.[210 Days]) AS [210 Days],
		SUM(#tmp_Summation.[240 Days]) AS [240 Days],
		SUM(#tmp_Summation.[270 Days]) AS [270 Days],
		SUM(#tmp_Summation.[300 Days]) AS [300 Days],
		SUM(#tmp_Summation.[330 Days]) AS [330 Days],
		SUM(#tmp_Summation.[360 Days]) AS [360 Days],
		SUM(#tmp_Summation.[2+ Years]) AS [2+ Years],
