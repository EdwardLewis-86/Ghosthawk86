...
isnull(CMI_ExcessAmount,0) as CWI_ExcessAmount, isnull(CMI_ExcessFunderAmount,0) as  CWI_ExcessFunderAmount ,CMI_QuoteAmount,	CMI_ClaimType_ID
--select *
...
CPA_PolicyPABasicItem_ID  ,
 ...
 0 as CWI_ExcessAmount,0 as  CWI_ExcessFunderAmount ,CPA_QuoteAmount,	CPA_ClaimType_ID
...
null as CLB_Name,null as 	CLB_Surname, null as 	CLB_Initials, null as 	CLB_IDNumber,
0 as CWI_ExcessAmount,0 as  CWI_ExcessFunderAmount ,CGI_QuoteAmount,	CGI_ClaimType_ID
...
null as CLB_Name,null as 	CLB_Surname, null as 	CLB_Initials, null as 	CLB_IDNumber,
0 as CWI_ExcessAmount,0 as  CWI_ExcessFunderAmount ,CCS_QuoteAmount,	CCS_ClaimType_ID
...
0 as CWI_ExcessAmount,0 as  CWI_ExcessFunderAmount ,CLB_QuoteAmount,	CLB_ClaimType_ID
...
CWI_ExcessAmount, CWI_ExcessFunderAmount ,CWI_QuoteAmount,	CWI_ClaimType_ID
...
SELECT  [Claim_ID]
		...
		,clb.CWI_ExcessAmount
		,clb.CWI_ExcessFunderAmount
		,case when  ROW_NUMBER() over (partition by CIC_ClaimItem_ID order by case when cic.CIC_AdditionalDescription = 'Assessment' then '2050-01-01' else   CIC_createdate end)  = 1 then  clb.CMI_QuoteAmount  else 0 end as    CMI_QuoteAmount
		,rct.CTI_Description
		,case when  ROW_NUMBER() over (partition by CIC_ClaimItem_ID order by case when cic.CIC_AdditionalDescription = 'Assessment' then '2050-01-01' else   CIC_createdate end)  = 1 then  CIS_SubAuthAmount  else 0 end as CIS_SubAuthAmount
		,case when  ROW_NUMBER() over (partition by CIC_ClaimItem_ID order by case when cic.CIC_AdditionalDescription = 'Assessment' then '2050-01-01' else   CIC_createdate end)  = 1 then  CIS_AuthAmount else 0 end as CIS_AuthAmount
	  ...
	  ,ROW_NUMBER() over (partition by CIC_ClaimItem_ID order by case when cic.CIC_AdditionalDescription = 'Assessment' then '2050-01-01' else   CIC_createdate end) as CIC_Seq
	  ,CIC_Description
 	  ,CMI_SectionName
	  ,cic.CIC_CreateDate
	  ,CIC_Limit
  ...
  into #CL_Claims
  --select ROW_NUMBER() over (partition by CIC_ClaimItem_ID order by case when cic.CIC_AdditionalDescription = 'Assessment' then '2050-01-01' else   CIC_createdate end) ,  cIC.*
  FROM  .[dbo].[Claim] as C
  ...
  left join [Evolve].[dbo].[ReferenceClaimType] as RCT on RCT.ReferenceClaimType_ID = clb.CMI_ClaimType_ID
  left outer join  .[dbo].[ClaimItemComponents] as CIC on CIC.CIC_ClaimItem_ID = CLB.ClaimMotorBasicItem_ID AND CIC_DELETED = 0
   left outer join [Evolve].[dbo].[ClaimItemSummary] as CS on CS.CIS_ClaimItem_ID = clb.ClaimMotorBasicItem_ID 
...
-- and [CLM_ClaimNumber] = 'DWA048889CLM'
  --and CLM_Deleted = 0
	...
	-- select *
	 from #CL_Claims as C
	 inner join  .[dbo].[ClaimSupplierLink] as cSl on C.ClaimMotorBasicItem_ID = csl.CSL_ClaimItem_ID  and  nullif(CIC_PayeeLink_ID,'') = csl.ClaimSupplierLink_ID
  ...
  --where [CLM_ClaimNumber] = 'DWA048889CLM'
...
select 4, 'SASRIA'
 ...
 Claim_Status, Claim_Item_Status 
 	, CWI_ExcessAmount
		, CWI_ExcessFunderAmount
		, case when supplier_type = 'Assessor' then 0 else   CMI_QuoteAmount  end as  CMI_QuoteAmount
		, CTI_Description
		,case when supplier_type = 'Assessor' then 0 else   CIS_SubAuthAmount  end as CIS_SubAuthAmount
		, case when supplier_type = 'Assessor' then 0 else  CIS_AuthAmount end as CIS_AuthAmount
 ,cast(null as varchar(250)) as Claim_Rejection_reason ,
 CLM_ReportedDate,	CLM_LossDate,CLM_CreateDate ,
 Payee_Type ,
 CIT_PayeeID,	CIT_PayeeName,
 supplier_type,
  Claim_Item_Sub_Status,CIC_Description,CIC_AdditionalDescription,  CMI_SectionName ,
  CLM_Description,
  CMI_ClaimDescription,
 ...
 from (
  ...
  C.Claim_ID,CIC_Description  , CIC_AdditionalDescription,   CMI_SectionName ,CMI_ClaimDescription ,CMI_OdoMeterReading,
  ...
  , CIT_PayeeID,	APT_Description as Payee_Type,CIT_PayeeType_ID,
   CIT_PayeeName,
    isnull(rst.Sup_Description,c.supplier_type) as  supplier_type,
...
left join   .[dbo].[Supplier] as SUP on   CIT_PayeeID  = sup.[Supplier_Id]
     left join   .[dbo].[ReferenceSupplierType] as RST on sup.SUP_Type = rst.[SupplierType_ID]
where 1=1
 --and CLM_ClaimNumber = 'DWA048889CLM'
