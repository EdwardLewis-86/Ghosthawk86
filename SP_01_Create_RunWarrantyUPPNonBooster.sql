...
-- Description: This stored procedure computes the Warranty None Booster for UPP
-- Version: 1.0.1
-- ================================================================================
-- =====================================================
-- CREATE STORED PROCEDURE: sp_RunWarrantyUPPNonBooster
-- =====================================================
-- This wraps the entire 2.1 script logic into a callable stored procedure
-- =====================================================
...
DROP PROCEDURE IF EXISTS dbo.sp_RunWarrantyUPPNonBooster;
GO


CREATE PROCEDURE dbo.sp_RunWarrantyUPPNonBooster
    @ValuationMonth DATE,
    @Commission FLOAT = 0.125,
    @ClaimsBinder FLOAT = 0.444444444,  -- Changed from 4 * 1.0000000 / 9
    @Binder FLOAT = 0.09,
    @OutputTableName VARCHAR(100)  -- e.g., 'SAW_UPP_202509_NonBooster'
AS
BEGIN
    SET NOCOUNT ON;
    ...
    PRINT '========================================';
    PRINT 'sp_RunWarrantyUPPNonBooster - Starting';
    PRINT 'Valuation Month: ' + CONVERT(VARCHAR, @ValuationMonth, 120);
    PRINT 'Output Table: ' + @OutputTableName;
...
-- ***************************************************************************
-- Clear logs to create space
--*****************************************************************************
Exec						master.dbo.ShrinkAllLogFiles;
...
Select						distinct p.Policy_ID,
							...
							Else
							Datediff(month, DATEADD(month, DATEDIFF(month, 0, p.POL_OriginalStartDate), 0), DATEADD(month, DATEDIFF(month, 0, @ValuationMonth), 0)) + 1 End ElapsedMonths,
							p.POL_SoldDate,
...
Where						1 = 1
							...
							and p.POL_ProductTerm_ID <> 4 -- Exclude monthlies
							and pv1.PRV_FullName not like '%Booster%';
							--and i.INS_InsurerName = 'Hollard Short Term';
							--and pol_policyNumber = 'HWTY035661POL';
...
--Fix 1: Correction of future active booster policies
Update #t2 set POL_OriginalStartDate = '2022-01-29 00:00:00.000', POL_StartDate = '2022-01-29 00:00:00.000', StartMonth = DATEADD(month, DATEDIFF(month, 0, '2022-01-29 00:00:00.000'), 0) where POL_PolicyNumber = 'QWTYM208190POL';
Update #t2 set POL_OriginalStartDate = '2022-01-21 00:00:00.000', POL_StartDate = '2022-01-21 00:00:00.000', StartMonth = DATEADD(month, DATEDIFF(month, 0, '2022-01-21 00:00:00.000'), 0) where POL_PolicyNumber = 'QWTYM304593POL';
Update #t2 set POL_OriginalStartDate = '2022-03-04 00:00:00.000', POL_StartDate = '2022-03-04 00:00:00.000', StartMonth = DATEADD(month, DATEDIFF(month, 0, '2022-03-04 00:00:00.000'), 0) where POL_PolicyNumber = 'QWTYM302682POL';
Update #t2 set POL_OriginalStartDate = '2021-08-01 00:00:00.000', POL_StartDate = '2021-08-01 00:00:00.000', StartMonth = DATEADD(month, DATEDIFF(month, 0, '2021-08-01 00:00:00.000'), 0) where POL_PolicyNumber = 'QWTYM304546POL';
...
-- Fix: Policy with no earning curve linked to it: QWTYM279861POL.
Update						#t2 
Set							PhasingCurves = 1, 
							POL_ProductTerm_ID = 2, 
							Term =24, 
							PhasingCurve1 = '24M - Even ALL V0',
							PhasingCurve2 = '24M - Even ALL V0' 
Where						POL_PolicyNumber = 'QWTYM279861POL';

-- Fix: Policy with no earning curve linked to it: QWTYM167988POL.
Update						#t2 
Set							PhasingCurves = 1, 
							POL_ProductTerm_ID = 7, 
							Term = 60, 
							PhasingCurve1 = '60M - Even ALL V0',
							PhasingCurve2 = '60M - Even ALL V0' 
Where						POL_PolicyNumber = 'QWTYM167988POL';

-- Fix: Policy with no earning curve linked to it: QWTYM018188POL.
Update						#t2 
Set							PhasingCurves = 1, 
							POL_ProductTerm_ID = 9, 
							Term = 60, 
							PhasingCurve1 = '60M - Even ALL V0',
							PhasingCurve2 = '60M - Even ALL V0' 
Where						POL_PolicyNumber = 'QWTYM018188POL';

-- Fix: Policy with no earning curve linked to it: HWTYM011438POL-01.
Update						#t2 
Set							PhasingCurves = 1, 
							PhasingCurve1 = '60M - Even ALL V0',
							PhasingCurve2 = '60M - Even ALL V0' 
Where						POL_PolicyNumber = 'HWTYM011438POL-01';
...
Select						#t2.*,
							...
							When lower(#t2.PhasingCurve2) like '%cnv%' then (ElapsedMonths * 1.000000 / Term)
							Else
							(Select Sum(DCI_MonthlyPercentage * 1.000000 / 100)
							From Evolve.dbo.DisbursementCurveItem dci, Evolve.dbo.DisbursementCurveHeader dch
							Where dci.DCI_DisbursementCurveHeader_Id = dch.DisbursementCurveHeader_Id
							and dch.DCH_Name = #t2.PhasingCurve2
							and dci.DCI_Month <= ElapsedMonths
							) End EarnedPortion,
							Case
							When ElapsedMonths = 0 then 1
							When ElapsedMonths > Term then 0
							When lower(#t2.PhasingCurve2) like '%cnv%' then 1 - (ElapsedMonths * 1.000000 / Term)
							Else
...
-- Comibne the various data sources
Select						#t9.*,
							bc.Binder WW_Binder,
							...
							af.FundExclVaT,
							af.FundPerc,
							tr.FundExclVaT TR_Fund,
							tr.CommExclVaT TR_Comm,
							tr.PremiumExclVaT TR_Premium
Into						#t10
From						#t9
                            ...
                            left join TYreAndRim.[dbo].[TyreAndRim_Fund] tr
							on tr.Pol_PolicyNumber = #t9.POL_PolicyNumber;
...
---- Fix 1: Migrated policy that has no account transaction information: 
Update						#t10 
...
---- Fix 2: Correct the binder fee disbursement which is negative .
Update						#t10 
...
Select						#t10.*,
							...
							when ElapsedMonths = 0 then 1
							Else #t10.UnearnedPortion End 
							*
							ISNULL(WW_Nett_Fund, Isnull(FundExclVaT, Evolve_Fund)) UnearnedFund,
							case when ElapsedMonths > Term then 0
							when ElapsedMonths = 0 then 1
							Else #t10.UnearnedPortion End 
							*
							Case when POL_PolicyNumber like 'QTYRM%' then TR_Fund else
							ISNULL(WW_Nett_Fund, Isnull(FundExclVaT, Evolve_Fund)) end UPP,
							case when ElapsedMonths > Term then 0
							when ElapsedMonths = 0 then 1
							Else 1 - (ElapsedMonths * 1.000000 / Term) End 
							*
							Case when POL_PolicyNumber like 'QTYRM%' then TR_Comm else
							ISNULL(WW_Commission, Isnull(EvolvePremiumExcVaTUpdated, case when Evolve_Premium < 0 Then 0 Else Evolve_Premium end) * @Commission) end
							*
							...
							* Case when INS_InsurerName = 'Centriq Short Term' then 1 else 0 end 
							...
							Else 1 - (ElapsedMonths * 1.000000 / Term) End 
							*
							ISNULL(WW_Nett_Comm, Evolve_Commission) 
							*
							Isnull(1 - WW_IsMigrated, 1)
							* Case when INS_InsurerName = 'Centriq Short Term' then 1 else 0 end 
							DAC_Comm,
							...
							* Case when INS_InsurerName = 'Centriq Short Term' then 1 else 0 end 
							...
							case when ElapsedMonths > Term then 0
							when ElapsedMonths = 0 then 1
							Else e.UnearnedPortion End 
							*
							Case when POL_PolicyNumber like 'QTYRM%' then TR_Premium * @Binder else
							ISNULL(WW_Binder, Isnull(EvolvePremiumExcVaTUpdated, Case When Evolve_Premium < 0 Then 0 Else Evolve_Premium end ) * @Binder) end 
							* @ClaimsBinder * Isnull(1 - WW_IsMigrated, 1) BinderUPP,
							case when ElapsedMonths > Term then 0
							when ElapsedMonths = 0 then 1
							Else #t10.UnearnedPortion End
							*
							ISNULL(WW_Roadside, Isnull(Case when Term = 24 and EvolvePremiumExcVaTUpdated is not null then 399 * 1.0000 / 1.15 when term = 12 and EvolvePremiumExcVaTUpdated is not null then 0.5 * 399 * 1.0000 / 1.15 when term = 6 and EvolvePremiumExcVaTUpdated is not null then 0.25 * 399 * 1.0000 / 1.15 else null end, Case When Evolve_Roadside < 0 Then 0 Else Evolve_Roadside end )) 
							 *
							Case when CellCaptive in ('AMH','Motus Imports') then 0 else 1 end
							RoadsideUPP
Into                        #FinalResult
From						#t10
                            left join [dbo].[Ecurves1] e
					        on CAST(#t10.Term as varchar(50)) = e.Curve
					        and #t10.ElapsedMonths = e.Month
where                       POL_EndDate >= @ValuationMonth;
...
----1. Check if there are future active Booster policies
Select						* 
Into						Booster_StartDate
From						#t10 
Where						product_level1  like '%Booster%' 
							and pol_startDate > eomonth(ValuationMonth);


--*********************************************************************************************
-- Export file using Dynamic SQL
--*********************************************************************************************

-- Build the dynamic SQL to create the output table directly from #FinalResult
SET @SQL = N'
SELECT						Policy_ID,
                            POL_PolicyNumber,
                            WW_Policy_Key,
                            ...
                            ValuationMonth,
                            ElapsedMonths,
                            CASE WHEN POL_SoldDate > ValuationMonth THEN POL_CreateDate ELSE POL_SoldDate END AS Pol_SoldDate,	
                            CalculatedEndDate3 AS POL_EndDate,
                            Product_Level1,
                            Product_Level2,
                            Product_Level3,
                            CellCaptive,
                            INS_InsurerName,
                            Term,
                            PhasingCurve2 AS PhasingCurve,
                            Agt_Name,
                            EarnedPortion,
                            UnearnedPortion,
                            WW_Nett_Premium,
                            WW_Nett_Fund,
                            WW_Binder AS WW_Nett_Binder,
                            WW_Commission AS WW_Nett_Commission,
                            WW_Roadside,
                            WW_IsMigrated,
                            CASE WHEN POL_PolicyNumber LIKE ''QTYRM%'' THEN TR_Premium 
                                 ELSE ISNULL(EvolvePremiumExcVaTUpdated, Evolve_Premium) 
                            END AS Evolve_Nett_Premium,
                            CASE WHEN POL_PolicyNumber LIKE ''QTYRM%'' THEN TR_Fund 
                                 ELSE ISNULL(FundExclVaT, Evolve_Fund) 
                            END AS Evolve_Nett_Fund,
                            CASE WHEN POL_PolicyNumber LIKE ''QTYRM%'' THEN TR_Premium * ' + CAST(@Binder AS VARCHAR(20)) + N' 
                                 ELSE Evolve_Binder 
                            END AS Evolve_Nett_Binder,
                            Evolve_Commission AS Evolve_Nett_Commission,
                            Evolve_Roadside,
                            UnearnedFund,
                            UPP,	
                            DAC,
...
INTO						[UPP].[dbo].[' + @OutputTableName + N']
FROM						#FinalResult
WHERE						UPP IS NOT NULL
                            AND UPP >= 0;';
...
SELECT @RowCount = COUNT(*) FROM #FinalResult WHERE UPP IS NOT NULL AND UPP >= 0;

PRINT '';
PRINT '========================================';
PRINT 'sp_RunWarrantyUPPNonBooster - Complete';
PRINT 'Output Table: ' + @OutputTableName;
PRINT 'Rows Created: ' + CAST(@RowCount AS VARCHAR);
