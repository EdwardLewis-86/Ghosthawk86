...
-- Description: This stored procedure computes the Warranty Booster for UPP
-- Version: 1.0.1
-- ================================================================================
-- =====================================================
-- CREATE STORED PROCEDURE: sp_RunWarrantyUPPBooster
-- Run this script ONCE to create the stored procedure
-- =====================================================
-- This wraps the entire 2.2 script logic into a callable stored procedure
-- =====================================================
...
IF OBJECT_ID('dbo.sp_RunWarrantyUPPBooster', 'P') IS NOT NULL
    DROP PROCEDURE dbo.sp_RunWarrantyUPPBooster;
GO

CREATE PROCEDURE dbo.sp_RunWarrantyUPPBooster
    @ValuationMonth DATE,
    @Commission FLOAT = 0.125,
    @ClaimsBinder FLOAT = 0.444444444,  -- 4 * 1.0000000 / 9
    @Binder FLOAT = 0.09,
    @OutputTableName VARCHAR(100)  -- e.g., 'SAW_UPP_202509_Booster'
AS
BEGIN
    SET NOCOUNT ON;
    ...
    PRINT '========================================';
    PRINT 'sp_RunWarrantyUPPBooster - Starting';
...
IF NOT EXISTS (SELECT 1
               FROM Evolve.sys.indexes
               WHERE name = 'a1' AND object_id = OBJECT_ID('Evolve.dbo.Policy'))
BEGIN
    CREATE NONCLUSTERED INDEX a1
    ON Evolve.[dbo].[Policy] ([POL_Status],[POL_PaymentFrequency_ID])
    INCLUDE ([POL_ProductTerm_ID]);
END

IF NOT EXISTS (SELECT 1
               FROM Evolve.sys.indexes
               WHERE name = 'a2' AND object_id = OBJECT_ID('Evolve.dbo.PolicyMechanicalBreakdownItem'))
BEGIN
    CREATE NONCLUSTERED INDEX a2
    ON Evolve.[dbo].[PolicyMechanicalBreakdownItem] ([PMI_Policy_ID])
...
--*********************************************************************************************
-- Export file using dynamic SQL to parameterize output table name
--*********************************************************************************************

	-- Use dynamic SQL to create the output table with parameterized name
	SET @SQL = N'
	SELECT Policy_ID,
			POL_PolicyNumber,
			WW_Policy_Key,
	...
	FROM #FinalResult
	WHERE UnearnedFund IS NOT NULL AND UnearnedFund >= 0;';
    ...
    PRINT '';
    PRINT '========================================';
    PRINT 'sp_RunWarrantyUPPBooster - Completed';
    PRINT 'End Time: ' + CONVERT(VARCHAR, @EndTime, 120);
    PRINT 'Duration: ' + CAST(@Duration / 60 AS VARCHAR) + ' minutes ' + CAST(@Duration % 60 AS VARCHAR) + ' seconds';
    PRINT '========================================';
