...
------------------------------------------------------------
-- Parameters (set your window)
------------------------------------------------------------
DECLARE @start date = '2019-01-01';
DECLARE @end   date = '2025-08-01';              -- exclusive upper bound
DECLARE @start_for_opening date = DATEADD(month,-1,@start);

-- Cash-out option (fixed to 2 per request)
...
--    Keep RAW product for exact per-product views; add PRD_Family for roll-ups
------------------------------------------------------------
SELECT
    p.Policy_ID,
    p.POL_PolicyNumber,
...
WHERE p.POL_Deleted = 0
  ...
  AND p.POL_Status IN (1,3)                                -- active-like statuses
  AND p.POL_OriginalStartDate < @end
  AND (p.POL_EndDate IS NULL OR p.POL_EndDate >= @start_for_opening);

CREATE NONCLUSTERED INDEX IX_Pol ON #Pol(POL_PolicyNumber, PRD_Name_RAW, PRD_Family, CellCaptive, CellCaptive2, CellGroup, Insurer);
...
------------------------------------------------------------
-- 3) EventLog -> monthly movement counters (NB/Reinst/EOT/Cancel/Exits)
--    NOTE: We no longer use last event to decide ClosingCount.
------------------------------------------------------------
SELECT
...
-- Policy × month, with date-based ClosingCount (in force at EOM)
SELECT
  p.POL_PolicyNumber, m.mthd,
  p.PRD_Name_RAW, p.PRD_Family, p.CellCaptive, p.CellCaptive2, p.CellGroup, p.Insurer,

  -- >>> NEW: state at EOM by dates & status <<<
  CASE
...
SELECT
  POL_PolicyNumber, mthd, PRD_Name_RAW, PRD_Family, CellCaptive, CellCaptive2, CellGroup, Insurer,
...
------------------------------------------------------------
-- 4) Finance (premiums & fees)  ->  #PolFin  (FAST/SARGABLE)
------------------------------------------------------------
...
SET @now = SYSDATETIME();
PRINT CONCAT('Step 4a (fast): GWP (#AT_Prem) done | step=', DATEDIFF(millisecond,@t_last,@now),' ms | total=', DATEDIFF(millisecond,@t0,@now),' ms');
SET @t_last = @now;
...
SET @now = SYSDATETIME();
PRINT CONCAT('Step 4b (fast): Fees (#AT_Fees) done | step=', DATEDIFF(millisecond,@t_last,@now),' ms | total=', DATEDIFF(millisecond,@t0,@now),' ms');
SET @t_last = @now;
...
SELECT
  po.POL_PolicyNumber,
  m.mthd,
  po.PRD_Name_RAW, po.PRD_Family, po.CellCaptive, po.CellCaptive2, po.CellGroup, po.Insurer,
...
------------------------------------------------------------
-- 5) Claims (OPTION 2: cash-out = Type 2)  (FAST/SARGABLE)
------------------------------------------------------------
...
-- debug
DECLARE @cntTxTotal bigint = (
  SELECT COUNT_BIG(*) FROM Evolve.dbo.ClaimItemTransaction cit
  WHERE cit.CIT_Deleted=0
    AND cit.CIT_TransactionType_ID = 2
    AND (
         (cit.CIT_EffectiveDate <  cit.CIT_postingDate AND cit.CIT_postingDate BETWEEN @start_for_opening AND DATEADD(day,-1,@end))
      OR (cit.CIT_EffectiveDate >= cit.CIT_postingDate AND cit.CIT_EffectiveDate BETWEEN @start_for_opening AND DATEADD(day,-1,@end))
    )
);
DECLARE @cntTxPay bigint = (SELECT COUNT_BIG(*) FROM #PayTx);
DECLARE @cntClaimsByPolMth bigint = (SELECT COUNT_BIG(*) FROM #ClaimsByPolMth);

SET @now = SYSDATETIME();
PRINT CONCAT('Step 5 (fast): Claims aggregate | TxInWindow=', @cntTxTotal,
             ' | PayType=', @CashOutTypeId,
             ' | PayTxRows=', @cntTxPay,
             ' | ClaimsByPolMth rows=', @cntClaimsByPolMth,
             ' | step=', DATEDIFF(millisecond,@t_last,@now),' ms | total=', DATEDIFF(millisecond,@t0,@now),' ms');
SET @t_last = @now;

------------------------------------------------------------
-- 6) Final monthly rollup (ABS amounts; filter to requested month)
--    NOTE: Group by RAW product so per-product ClosingCount makes sense.
------------------------------------------------------------
;WITH agg AS (
  SELECT
      pm.mthd,
  ...
  GROUP BY pm.mthd, pm.PRD_Name_RAW, pm.PRD_Family, pm.CellCaptive, pm.CellCaptive2, pm.CellGroup, pm.Insurer
)
SELECT
    a.mthd,
    a.PRD_Name,           -- RAW product
    a.PRD_Family,         -- optional roll-up (keep for convenience)
    a.CellCaptive,
    a.CellCaptive2,
    a.CellGroup,
    a.Insurer,
...
SELECT *
FROM #Final
ORDER BY PRD_Name, CellCaptive2, CellGroup, mthd;
