...
# Ensure that claim reason is the same as the SOB benefits otherwise the MCMC model will be incorrect
...
premium_per_policy = 5040.00  #average premium per policy if monthly x period of term

# Define the minimum claim value to siulate against SOB
min_SOB = 3000

# ------------------ Constants ------------------
term_product = False  # Set to True if premium is fixed term; False if monthly policy
max_premium_increase = 0.30  # Max increase per model
burn_reduction_target = 0.30  # Target reduction in burn rate
...
# Define output path
output_path = "C:/Users/edwardl/OneDrive - Motus Corporation/Documents/3. RStudio_Python Outputs/Existing Claim Data Analyses Kia 5 year"
QQ_output_path = "C:/Users/edwardl/OneDrive - Motus Corporation/Documents/3. RStudio_Python Outputs/Existing Claim Data Analyses Kia 5 year/QQ Plots"

# Define input path
input_path = "C:/Users/edwardl/OneDrive - Motus Corporation/Documents/2. RStudio_Python Inputs/Kia 5 year/"



# Define Generalized Beta of the Second Kind (GB2) distribution)
class GB2Gen(rv_continuous):
  def _pdf(self, x, a, b, p, q):
        return (a * x**(a * p - 1)) / (b**(a * p) * beta(p, q) * (1 + (x / b)**a)**(p + q))

# Define and add GB2 to the stats module
gb2 = GB2Gen(name="gb2", shapes="a, b, p, q", a=0)
...
with pd.ExcelWriter(excel_file, engine='xlsxwriter') as writer:
    ...
    _ =plt.barh(y_pos, total_claims_by_reason['Number_of_Claims'], color='skyblue')
    
    # Set y-tick labels
    _ =plt.yticks(y_pos, total_claims_by_reason['Reason for Claim'])
    
    _ =plt.xlabel("Number of Claims")
    _ =plt.title("Claim Count by Reason")
    _ =plt.tight_layout()
    _ =plt.savefig(chart1_path, dpi=300)
    _ =plt.close('all')
    ...
    _ =pivot_make = total_claims_by_make.pivot_table(
        index='Vehicle Make',
        columns='Reason for Claim',
        values='Number_of_Claims',
        fill_value=0
    )
    _ =pivot_make.plot(kind='barh', stacked=True, figsize=(14, 8), colormap='tab20')
    _ =plt.xlabel("Number of Claims")
    _ =plt.title("Claim Count by Reason and Vehicle Make")
    _ =plt.tight_layout()
    _ =plt.savefig(chart2_path, dpi=300)
    _ =plt.close('all')
    ...
    _ =sns.heatmap(total_claims_by_reason.set_index("Reason for Claim")[["Number_of_Claims"]],
                annot=True, fmt=".0f", cmap=sns.color_palette("RdYlGn_r", as_cmap=True))
    _ =plt.title("Claim Count by Reason")
    _ =plt.xlabel("")
    _ =plt.ylabel("Reason for Claim")
    _ =plt.tight_layout()
    _ =plt.savefig(chart4a_path, dpi=300)
    _ =plt.close('all')
    ...
    _ =sns.heatmap(pivot_make, annot=True, fmt=".0f", cmap=sns.color_palette("RdYlGn_r", as_cmap=True))
    _ =plt.title("Claim Count by Reason and Vehicle Make")
    _ =plt.xlabel("Reason for Claim")
    _ =plt.ylabel("Vehicle Make")
    _ =plt.tight_layout()
    _ =plt.savefig(chart4b_path, dpi=300)
    _ =plt.close('all')
    ...
    _ =heatmap_pivot = heatmap_data.pivot_table(
        index='Make_Model',
        columns='Reason for Claim',
        values='Number_of_Claims',
        fill_value=0
    ...
    _ =plt.figure(figsize=(16, 10))
    _ =sns.heatmap(heatmap_pivot, annot=True, fmt=".0f", cmap=sns.color_palette("RdYlGn_r", as_cmap=True))
    _ =plt.title("Claim Count by Reason and Top 20 Vehicle Models")
    _ =plt.xlabel("Reason for Claim")
    _ =plt.ylabel("Vehicle Make & Model")
    _ =plt.tight_layout()
    _ =plt.savefig(chart4c_path, dpi=300)
    _ =plt.close('all')
...
with pd.ExcelWriter(excel_file, engine="xlsxwriter") as writer:
    ...
    _ =y_pos = range(len(reason_freq_exposure))
    
    # Plot bars using numeric positions
    _ =plt.barh(y_pos, reason_freq_exposure["Annualized Claim Frequency (%)"], color="skyblue")
    
    # Set y-tick labels to string values
    _ =plt.yticks(y_pos, reason_freq_exposure["Reason for Claim"])
    
    _ =plt.xlabel("Annualized Claim Frequency (%)")
    _ =plt.title("Annualized Claim Frequency by Reason")
    _ =plt.tight_layout()
    ...
    _ =pivot_make.plot(kind="barh", stacked=True, colormap="tab20")
    _ =plt.xlabel("Annualized Claim Frequency (%)")
    _ =plt.title("Annualized Claim Frequency by Reason and Vehicle Make")
    _ =plt.tight_layout()
    ...
    _ =plt.figure(figsize=(12, 8))
    _ =sns.heatmap(reason_freq_exposure.set_index("Reason for Claim")[["Annualized Claim Frequency (%)"]], 
           annot=True, fmt=".1f", cmap=sns.color_palette("RdYlGn_r", as_cmap=True))
    _ =plt.title("Annualized Claim Frequency (%) by Reason of Claim")
    _ =plt.xlabel("")
    _ =plt.ylabel("Reason for Claim")
    _ =plt.tight_layout()
    _ =plt.savefig(chart4a_path, dpi=300)
    ...
    _ =plt.figure(figsize=(16, 10))
    _ =sns.heatmap(pivot_make, annot=True, fmt=".1f", cmap=sns.color_palette("RdYlGn_r", as_cmap=True))
    _ =plt.title("Annualized Claim Frequency (%) by Reason and Vehicle Make")
    _ =plt.xlabel("Reason for Claim")
    _ =plt.ylabel("Vehicle Make")
    _ =plt.tight_layout()
    ...
    _ =plt.figure(figsize=(16, 10))
    _ =sns.heatmap(heatmap_pivot, annot=True, fmt=".1f", cmap=sns.color_palette("RdYlGn_r", as_cmap=True))
    _ =plt.title("Final Annualized Claim Frequency (%) by Reason and Top 20 Vehicle Models")
    _ =plt.xlabel("Reason for Claim")
    _ =plt.ylabel("Vehicle Make & Model")
    _ =plt.tight_layout()
    _ =plt.savefig(chart4c_path, dpi=300)
...
data_path = "C:/Users/edwardl/OneDrive - Motus Corporation/Documents/2. RStudio_Python Inputs/Kia 5 year/claims_policy_data.xlsx"
...
for name, data in transformations.items():
    ...
    _ =sns.histplot(data, bins=50, color="grey", alpha=0.6, kde=False, stat="density", label=f"{name} Transformation")

    # Plot KDE separately (blue color) but exclude label from automatic legend
    _ =sns.kdeplot(data, color="blue", linewidth=2, label=None)

    # Compute histogram values
    _ =hist_values, bin_edges = np.histogram(data, bins=50, density=True)
    _ =bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2  # Bin centers

    # Compute moving average for smoothing
    _ =window_size = 5  # Adjust for more/less smoothing
    _ =smoothed_trend = np.convolve(hist_values, np.ones(window_size) / window_size, mode='same')

    # Plot smoothed trend line in red
    _ =plt.plot(bin_centers, smoothed_trend, color="red", linewidth=2, linestyle="dashed", label=None)

    # Create custom legend handles
    _ =kde_line = mlines.Line2D([], [], color='blue', linewidth=2, label="KDE (Density Estimate)")
    _ =trend_line = mlines.Line2D([], [], color='red', linestyle="dashed", linewidth=2, label="Smoothed Trend Line")
    _ =hist_patch = mlines.Line2D([], [], color='grey', linewidth=8, alpha=0.6, label="Histogram Bins")

    # Add legend
    _ =plt.legend(handles=[hist_patch, kde_line, trend_line])

    # Labels and title
    _ =plt.title(f"{name} Claim Amount Distribution")
    _ =plt.xlabel("Transformed Claim Amount")
    _ =plt.ylabel("Density")
    _ =plt.grid(True)

    # Save plot
    transformed_output_path = os.path.join(output_path, f"{name}_Claim_Distribution.png")
    _ =plt.savefig(transformed_output_path, dpi=300, bbox_inches='tight')
    _ =plt.close("all")
...
_ =plt.hist(raw_claims, bins=50, alpha=0.6, color='grey', label='Raw Claims (Counts)', density=False)
...
_ =plt.plot(x, bell_curve, color='red', linewidth=2, label='Normal Distribution (Bell Curve)')

_ =plt.title("Raw Claim Amounts with Bell Curve")
_ =plt.xlabel("Claim Amount")
_ =plt.ylabel("Counts")
_ =plt.legend()
_ =plt.grid(True)
...
_ =plt.savefig(histogram_output_path, dpi=300, bbox_inches='tight')
_ =plt.close("all")
...
_ =plt.scatter(range(len(raw_claims)), raw_claims, alpha=0.5, color='blue', label="Data")
_ =plt.scatter(outlier_indices, outlier_values, color='red', label="Outliers")  # Highlight outliers in red

# Plot mean and standard deviation lines
_ =plt.axhline(raw_mean, color='black', linestyle='dashed', label="Mean")
_ =plt.axhline(lower_bound_1std, color='green', linestyle='dotted', label="-1 Std Dev")
_ =plt.axhline(upper_bound_1std, color='green', linestyle='dotted', label="+1 Std Dev")
_ =plt.axhline(lower_bound_2std, color='red', linestyle='dotted', label="-2 Std Dev")
_ =plt.axhline(upper_bound_2std, color='red', linestyle='dotted', label="+2 Std Dev")

# Labels and title
_ =plt.legend()
_ =plt.title("Scatter Plot of Raw Claim Amounts with Outliers")
_ =plt.xlabel("Index")
_ =plt.ylabel("Claim Amount (Rands)")

# Save the plot
outlier_plot_path = os.path.join(output_path, "Outlier_Scatter_Plot.png")
_ =plt.savefig(outlier_plot_path, dpi=300, bbox_inches='tight')
_ =plt.close("all")
...
for _, row in best_fit_df.iterrows():
    ...
    try:
        ...
        _ = plt.scatter(theoretical_quantiles, sorted_data, alpha=0.5, label="Observed Data")
        _ = plt.plot(
            [min(theoretical_quantiles), max(theoretical_quantiles)],
            [min(theoretical_quantiles), max(theoretical_quantiles)],
            color='red', linestyle='--', label='Ideal Fit'
        )
        _ = plt.xlabel(f"Theoretical Quantiles ({distribution_name})")
        _ = plt.ylabel(f"Observed Quantiles ({transformation})")
        _ = plt.title(f"QQ Plot - {transformation} ({distribution_name})")
        _ = plt.legend()
        _ = plt.grid(True)

        # Save QQ plot
        qq_filename = os.path.join(QQ_output_path, f"QQ_{transformation}_{distribution_name}.png")
        _ =plt.savefig(qq_filename, dpi=300, bbox_inches='tight')
        _ =plt.close("all")
...
_ =plt.figure(figsize=(10, 6))
_ =sns.histplot(simulated_claims_df["Claim Cost"], bins=50, kde=True, color="blue", alpha=0.6)
_ =sns.regplot(x=np.arange(len(simulated_claims_df)), y=simulated_claims_df["Claim Cost"], scatter=False, color="red")

# Add labels and title
_ =plt.title("Histogram of Simulated Claim Costs with KDE and Linear Trend")
_ =plt.xlabel("Claim Cost")
_ =plt.ylabel("Number of Claims")

# Save the plot to the output directory
_ =plt.savefig(histogram_plot_path, dpi=300, bbox_inches='tight')
_ =plt.close("all")  # Close the figure to free memory
...
# ----------------------- Plot VaR and TVaR -----------------------
_ =plt.figure(figsize=(10, 6))
_ =sns.kdeplot(claim_amounts, fill=True, label='Claim Distribution')

# Plot VaR
_ =plt.axvline(var_results[0.95], color='red', linestyle='--', label='VaR 95%')
_ =plt.axvline(var_results[0.99], color='blue', linestyle='--', label='VaR 99%')

# Plot TVaR
_ =plt.axvline(tvar_results[0.95], color='green', linestyle='-.', label='TVaR 95%')
_ =plt.axvline(tvar_results[0.99], color='purple', linestyle='-.', label='TVaR 99%')

# Ensure legend and title
_ =plt.legend()
_ =plt.title('Value at Risk and Tail Value at Risk')
_ =plt.xlabel('Claim Cost')
_ =plt.ylabel('Density')

# Save plot
_ =plot_output_path =os.path.join(output_path,"Risk Analysis VaR TVar Plot.png")
_ =plt.savefig(plot_output_path)
_ =plt.close("all")
...
_ =x_actual = np.linspace(min(raw_claims), max(raw_claims), 1000)
_ =x_simulated = np.linspace(min(simulated_claims), max(simulated_claims), 1000)

# Kernel Density Estimation (KDE) for actual and simulated claims
_ =kde_actual = stats.gaussian_kde(raw_claims)
_ =kde_pdf_actual = kde_actual(x_actual)

_ =kde_simulated = stats.gaussian_kde(simulated_claims)
_ =kde_pdf_simulated = kde_simulated(x_simulated)

# ---------------------- Plot histograms with borders and KDE ----------------------
_ =plt.figure(figsize=(14, 8))

# Histogram for actual claims with black border
_ =plt.hist(raw_claims, bins=50, alpha=0.6, color='grey', edgecolor='black', density=True, label="Actual Data")

# Histogram for simulated claims with black border
_ =plt.hist(simulated_claims, bins=50, alpha=0.4, color='red', edgecolor='black', density=True, label="Simulated Data")

# KDE plot for actual claims
_ =plt.plot(x_actual, kde_pdf_actual, color='black', linestyle='solid', label="Actual KDE")

# KDE plot for simulated claims
_ =plt.plot(x_simulated, kde_pdf_simulated, color='red', linestyle='dashed', label="Simulated KDE")

# Dynamically set y-axis based on max of both KDEs
_ =y_max_combined = max(np.max(kde_pdf_actual), np.max(kde_pdf_simulated)) * 1.1  # Add 10% buffer
_ =plt.ylim(0, y_max_combined)

_ =plt.title("Gaussian Mixture Model - Actual vs Simulated Claims with Bins & KDE")
_ =plt.xlabel("Claim Amount")
_ =plt.ylabel("Density")
_ =plt.legend()
...
_ =plt.gca().xaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f'{int(x/1000):,}k'))
_ =plt.savefig(gmm_binned_kde_plot_path, dpi=300, bbox_inches='tight')
...
# Plot non-outliers
_ =plt.scatter(non_outliers.index, non_outliers, label='Data', color='blue', alpha=0.6)

# Plot outliers
_ =plt.scatter(outliers.index, outliers, label='Outliers', color='red', alpha=0.8)

# Plot mean and standard deviation lines
_ =plt.axhline(mean_cost, color='black', linestyle='--', label='Mean')
_ =plt.axhline(mean_cost + std_cost, color='green', linestyle='dotted', label='+1 Std Dev')
_ =plt.axhline(mean_cost - std_cost, color='green', linestyle='dotted', label='-1 Std Dev')
_ =plt.axhline(mean_cost + 2 * std_cost, color='red', linestyle='dotted', label='+2 Std Dev')
_ =plt.axhline(mean_cost - 2 * std_cost, color='red', linestyle='dotted', label='-2 Std Dev')

# Format plot
_ =plt.title("Scatter Plot of Claim Cost with Outliers Highlighted")
_ =plt.xlabel("Index")
_ =plt.ylabel("Claim Cost (Rands)")
_ =plt.legend()
_ =plt.grid(True)
_ =plt.tight_layout()
_ =scatter_plot_path = os.path.join(output_path, "Simulated_scatter_plot.png")
_ =plt.savefig(scatter_plot_path, dpi=300, bbox_inches='tight')

# trace plot
_ =plt.figure(figsize=(14, 5))
_ =plt.plot(simulated_claims, color='blue', alpha=0.6)
_ =plt.title("MCMC Trace Plot of Simulated Claim Costs")
_ =plt.xlabel("Iteration")
_ =plt.ylabel("Claim Cost (Rands)")
_ =plt.grid(True)
_ =plt.tight_layout()
_ =trace_plot_path = os.path.join(output_path, "MCMC_trace_plot.png")
_ =plt.savefig(trace_plot_path, dpi=300, bbox_inches='tight')


#Posterior
_ =plt.figure(figsize=(10, 6))
_ =plt.hist(simulated_claims, bins=50, color='skyblue', edgecolor='black', density=True)
_ =plt.title("Posterior Distribution of Simulated Claim Costs")
_ =plt.xlabel("Claim Cost (Rands)")
_ =plt.ylabel("Density")
_ =plt.grid(True)
_ =plt.tight_layout()
_ =posterior_plot_path = os.path.join(output_path, "MCMC_posterior_distribution.png")
_ =plt.savefig(posterior_plot_path, dpi=300, bbox_inches='tight')





# ✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅✅Pricing Adequacy & Premium Adjustment Script
# ------------------ Load Data ------------------
# Premium table
premium_file = os.path.join(input_path, "claims_policy_data.xlsx")
df_premiums = pd.read_excel(premium_file)
df_premiums.columns = df_premiums.columns.str.strip()

# Claims + Policy Data
claims_df = pd.read_csv(os.path.join(output_path, "Imported_Claims_Data.csv"))
claims_df["Expiry Date"] = pd.to_datetime(claims_df["Expiry Date"], errors='coerce')
claims_df["Inception Date"] = pd.to_datetime(claims_df["Inception Date"], errors='coerce')

# Simulated inflated claims (used as Paid Claims)
simulated_df = pd.read_csv(os.path.join(output_path, "6.1 Simulated Claims.csv"))

# ------------------ Merge Premiums ------------------
premium_cols = ["Vehicle Make", "Vehicle Model", "Plan", "Premium"]
df_premiums = df_premiums[premium_cols]

# Policy Duration in Months
claims_df["Adjusted Expiry"] = claims_df["Expiry Date"].fillna(pd.Timestamp.today())
claims_df["Exposure Months"] = (
    (np.minimum(claims_df["Adjusted Expiry"], pd.Timestamp.today()) - claims_df["Inception Date"]).dt.days / 30.4375
).clip(lower=0)
...
merged_df = pd.merge(
    claims_df,
    df_premiums,
    on=["Vehicle Make", "Vehicle Model", "Plan"],
    how="left"
)

# Monthly premiums
merged_df = merged_df.dropna(subset=["Premium"])
merged_df["Total Premium Collected"] = np.where(
    term_product,
    merged_df["Premium"],
    merged_df["Premium"] * merged_df["Exposure Months"]
)

# Claims Paid
claim_paid_df = simulated_df.groupby(["Vehicle Make", "Claim Reason"])["Claim Cost"].sum().reset_index()
claim_paid_df = claim_paid_df.rename(columns={"Claim Cost": "Total Claims Paid"})

# Aggregate metrics per Make-Model-Plan
agg = merged_df.groupby(["Vehicle Make", "Vehicle Model", "Plan"]).agg(
    Policies=("Policy number", "nunique"),
    Exposure_Months=("Exposure Months", "sum"),
    Total_Premium=("Total Premium Collected", "sum")
).reset_index()

# Merge in claims paid
agg = pd.merge(
    agg,
    simulated_df.groupby(["Vehicle Make"])["Claim Cost"].sum().reset_index().rename(columns={"Claim Cost": "Total Claims Paid"}),
    on="Vehicle Make",
    how="left"
)

agg["Total Claims Paid"] = agg["Total Claims Paid"].fillna(0)
agg["Burn Rate"] = agg["Total Claims Paid"] / agg["Total_Premium"].replace(0, np.nan)
agg["Burn Rate"] = agg["Burn Rate"].fillna(0)

# ------------------ GMM Clustering ------------------
gmm_input = agg[["Burn Rate"]].replace([np.inf, -np.inf], np.nan).fillna(0)
scaler = StandardScaler()
gmm_scaled = scaler.fit_transform(gmm_input)

gmm_model = GaussianMixture(n_components=5, random_state=42)
agg["GMM Component"] = gmm_model.fit_predict(gmm_scaled)

# Assign risk group names based on mean burn
component_order = agg.groupby("GMM Component")["Burn Rate"].mean().sort_values().index
risk_labels = ["Very Low", "Low", "Medium", "High", "Very High"]
component_mapping = dict(zip(component_order, risk_labels))
agg["GMM Category"] = agg["GMM Component"].map(component_mapping)

# ------------------ Premium Adjustment ------------------
current_total = agg["Total_Premium"].sum()
target_total = current_total * (1 - burn_reduction_target)

# Proposed new premium per unit
agg["Target Premium"] = agg["Total Claims Paid"] / (1 - burn_reduction_target)
agg["Unit Current Premium"] = agg["Total_Premium"] / agg["Exposure_Months"].replace(0, np.nan)
agg["Unit Target Premium"] = agg["Target Premium"] / agg["Exposure_Months"].replace(0, np.nan)

agg["Recommended Premium"] = np.minimum(
    agg["Unit Current Premium"] * (1 + max_premium_increase),
    agg["Unit Target Premium"]
)

agg["% Change"] = (agg["Recommended Premium"] - agg["Unit Current Premium"]) / agg["Unit Current Premium"] * 100
agg["% Change"] = agg["% Change"].clip(upper=30)

# ------------------ Output ------------------
adjustment_output = os.path.join(output_path, "7.1 Premium Adjustment Summary.xlsx")
with pd.ExcelWriter(adjustment_output, engine="xlsxwriter") as writer:
    agg.to_excel(writer, sheet_name="Premium Adjustment", index=False)

# ------------------ Plot: Premium Changes ------------------
plt.figure(figsize=(14, 6))
sns.histplot(agg["% Change"], bins=30, kde=True, color="steelblue")
plt.axvline(30, color='red', linestyle='--', label='30% Cap')
plt.title("Distribution of Premium Adjustments")
plt.xlabel("% Change in Premium")
plt.ylabel("Frequency")
plt.legend()
adjustment_plot = os.path.join(output_path, "7.3 Premium Adjustment Charts.png")
plt.tight_layout()
plt.savefig(adjustment_plot)
plt.close()

# ------------------ Plot: GMM Cluster ------------------
plt.figure(figsize=(10, 6))
sns.scatterplot(
    x="Unit Current Premium",
    y="Burn Rate",
    hue="GMM Category",
    data=agg,
    palette="Set1",
...
plt.title("GMM Clustering: Premium vs Burn Rate")
plt.xlabel("Current Monthly Premium")
plt.ylabel("Burn Rate")
plt.grid(True)
plt.legend()
gmm_plot = os.path.join(output_path, "7.2 GMM Cluster Plot.png")
plt.tight_layout()
plt.savefig(gmm_plot)
plt.close()

print(f"✅ Premium adequacy analysis and adjustments completed.")
